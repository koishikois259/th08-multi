...
- Each player has a selected-team header and individual LIFE red stars, BOMB
  blue stars, and `POWER value/128`. Shared rows follow the two groups. The
  grouped layout was visually confirmed by the user; the final POWER spacing
  still needs a visual check.

The VC7 multiplayer and normal builds pass. The grouped resource layout was
visible, but complete two-window HUD acceptance is still **PENDING**. Do not
...
- The v0.34 multiplayer build and focused state/network tests pass. The
  ...
  `0eff23a3db246aa4c453894e08b782bd86b3ee91281b8e22aa76c1aa88d48bf8`
...
- Two local v0.34 game windows were visible and both reported
  `connected (menus synchronized)`. The user accepted the grouped HUD layout;
  the final POWER spacing and cross-PC trusted LAN/VPN play need manual checks.
