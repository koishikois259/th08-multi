...
- library inventory: **1,112 classified functions**; all **1,112 / 1,112** now have
  mapping sizes totaling **216,256 bytes**; the independent library exact ledger currently accepts **25 functions / 3,227 body bytes**;
...
Library acceptance now has **25 / 25 configured units** and **3,227 body bytes**
...
`_nh_malloc @ 0x004A423D` / `malloc @ 0x004A4269` with explicit DIR32/REL32 replay, plus `__sbh_heap_init`, `__sbh_find_block`, and the 735-byte `__sbh_resize_block` from `sbheap.obj`.  Continue pinning provenance/match units for
