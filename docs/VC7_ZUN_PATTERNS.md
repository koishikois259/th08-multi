...
- RunEcl opcode 135's child-context zero fill is sensitive to the source loop form.  `memset` and simple index loops miss the target shape; a real pointer-range loop (`for (i32 *clear = child, *end = child + 0x92C; clear < end; ++clear)`) improved the aggregate crosswalk when paired with opcode 128.  Concrete `g_ZunMemory` owner binding for this handler was probed and rejected because it over-shrank the handler in the current stack layout.
