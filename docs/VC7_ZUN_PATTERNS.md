...
- More RunEcl singleton call-site binding: when the target loads a concrete manager into `ecx`, call that owner directly.  Verified exact forms include opcode 124's `g_SoundPlayer.PlaySoundPositionedByIdx`, opcode 139's `g_EffectManager.SpawnEffect`, opcode 141's `g_ItemManager.SpawnItem`, and opcode 184's `g_Spellcard.SetBonusUpdatesDisabled`.  Opcode 142 retains an out-of-line `g_GameManager.GetPower()` local because the direct condition inlines and over-shrinks.
...
- When a RunEcl call tail loads a concrete owner singleton, bind that owner directly.  Opcode 109 builds the descriptor position from the Enemy's position and shoot offset, then calls `g_BulletManager.SpawnBulletPattern`, preserving the target owner and exact handler span.

- RunEcl player-angle calls should bind the concrete `g_Player` singleton when the target loads `0x017D5EF8` into `ecx`.  Opcodes 68, 69, and 118 now call `g_Player.AngleToPoint(...)`; their formerly separate overlay declaration was retired after the public Player owner became exact.
