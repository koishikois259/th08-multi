...
- RunEcl opcode 140 currently benefits from making only operand 3's fallback store explicit as raw bits while leaving operands 4 and 5 on `TH08_ECL_READ_F_RAWARG`.  Applying the same explicit branch to every vector component over-shrinks the handler and worsens absolute delta.
...
- RunEcl opcode 79's bit replacement is not uniform: the inverted low bits should stay as ternary mask writes, but the positive bit writes for `0x10` and the secondary `0x40` bit should be written as boolean shifts (`(((lhsInt & mask) != 0) << shift)`).  Applying this form to the inverted low bits over-shrinks the handler.
