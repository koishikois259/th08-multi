...
| How do I build the normal executable? | `python3 scripts/build.py` | Regenerates `build.ninja` and links `build/th08.exe`. Add `--fresh` to clean only Ninja-declared outputs first. |
...
| Is the complete accepted ledger reproducible from a cold build? | `python3 scripts/analysis/verify-exact-units.py --all --json > build/accepted-unit-replay.json` | Cleans Ninja outputs, builds every configured object with one job, and replays all accepted units. Build logs go to stderr, so the report is pure JSON. |
