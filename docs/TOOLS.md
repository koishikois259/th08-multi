...
| What differs in the complete linked executable? | `python3 scripts/compare-whole-image.py --json > build/whole-image-report.json` | Verifies the canonical target, then reports PE headers/directories, section bytes, imports, resources, debug data, and accepted-unit linker-map address anchors. It is diagnostic and never edits an exact ledger. |
