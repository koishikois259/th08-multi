...
### Claim-safe exact promotion from an active foreign lane

- A function that is exact only in another agent's dirty/claimed translation unit must not be ledgered against that uncommitted object. Extract the minimal exact source body into a separate probe TU, preserve the claimed production TU untouched, and point the canonical match unit at the probe object. `TitleScreen::OnUpdateReplayMenu @ 0x0046E136` was promoted this way: the minimal probe needs only the function body, `g_StageNames`, `TITLE_MENU_ITEM_START_REPLAY`, and the small `InitializeTitleVmAndSetSprite` helper, yet still reproduces all 3671 bytes and 107 relocations.
