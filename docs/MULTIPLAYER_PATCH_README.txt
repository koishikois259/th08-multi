...
网络联机双方必须使用完全相同的 v0.4 补丁和相同版本的原版游戏数据。
...
本地单进程双控：在启动器选择 Local play (multi input)，用两台不同
输入设备依次按一次“确认键”，再将设备分别分配给 P1、P2，点击
Start local game。键盘的确认键是 Z；手柄使用确认/射击键，识别设备时
也接受其他手柄按钮以兼容自定义按键。支持双键盘、键盘加手柄或双手柄。
选人时先由 P1 确认，再由 P2 确认。本地模式目前仍需更多实体设备测试。

GitHub 源码：
https://github.com/koishikois259/th08-multi

本地 v0.4 补丁尚未上传 GitHub Release。
...
v0.4 本轮改动：
- 新增单进程、单窗口的本地双人模式，可分别绑定两台输入设备。
- 键盘和手柄均需按确认键后才会列入设备表；同一次按键被多个手柄
  接口上报时仅保留一个设备项。

继承自 v0.34 的主要改动：
- 双方初始残机数统一采用主机（P1）的游戏设置，忽略客机（P2）的本地设置；
  未修改设置时默认均为 2。
- 右侧按玩家分组显示所选队伍、LIFE 红色星号、BOMB 蓝色星号以及
  POWER 数值/128；POWER 与数值之间留一个字符的间隔。共享资源显示在
  两组玩家资源下方，星号保持标准尺寸（1.0 倍）。
- 对端机体远距离不透明度为 50%，距离小于 64 像素时为 10%；本端不再显示
  对端的白色低速判定点。
...
v0.4 包含 v0.32 的全部内存安全防护。仍建议在可信环境中对 4、5、6、EX 面进行
长时间双机测试。
