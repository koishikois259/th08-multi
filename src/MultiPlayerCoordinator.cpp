...
const u32 TH08_MULTI_BUILD_FINGERPRINT = 0x00030000;
    ...
    if (config.mode != MULTI_LAUNCH_DISABLED)
    {
        ...
        if (sessionState != displayedState && g_Supervisor.hwndGameWindow != NULL)
        {
            if (sessionState == MULTI_NET_STATE_LISTENING)
                wsprintfA(title, "th08-multi v0.3 - HOST waiting on UDP %u", config.localPort);
            else if (sessionState == MULTI_NET_STATE_CONNECTING)
                wsprintfA(title, "th08-multi v0.3 - connecting to %s:%u", config.hostAddress, config.hostPort);
            else if (sessionState == MULTI_NET_STATE_CONNECTED)
                wsprintfA(title, "th08-multi v0.3 - connected (select teams in game / delay %u)",
                          session.GetInputDelay());
            else
                wsprintfA(title, "th08-multi v0.3 - network error %u", session.GetError());
                                            ...
                                            i32 initialBombs, i32 initialPower)
{
    ...
    {
        wsprintfA(title, "th08-multi v0.3 - playing (P1 team %u / P2 team %u / delay %u)",
