...
void TitleScreen::FormatSpellCardInfo()
{
    ...
    if (this->currentScreenState == TitleCurrentScreenState_Ready && this->unk0xc29c == 0)
        return;
    ...
    if (this->currentScreenState == TitleCurrentScreenState_Init || this->unk0xc29c == 11)
    {
    ...
    if (this->currentScreenState == TitleCurrentScreenState_Init || this->unk0xc29c == 9)
    {
    ...
    if (Spellcard::GetDifficultyFromSpellCard(spellCardNumber) <= EXTRA)
    {
        ...
        {
            if (this->currentScreenState == TitleCurrentScreenState_Init || this->unk0xc29c == 7)
                g_AnmManager->DrawTextLeft(&this->spellCardInfoVms[3], COLOR_TEXT_WHITE, 0,
                    "\x81\x40\x81\x40---/---(---/---)[--------]\x81\x40\x81\x40---/---(---/---)[--------]");
        }
        else if (this->currentScreenState == TitleCurrentScreenState_Init || this->unk0xc29c == 7)
        {
            g_AnmManager->DrawTextLeft(&this->spellCardInfoVms[3], COLOR_TEXT_WHITE, 0,
    ...
    if (this->currentScreenState == TitleCurrentScreenState_Init || this->unk0xc29c == 5)
    {
        if (!reinterpret_cast<TitleCatkView *>(&g_GameManager.catkData[spellCardNumber])->AttemptedAny(SHOT_ALL) &&
    ...
    if (this->currentScreenState == TitleCurrentScreenState_Init || this->unk0xc29c == 3)
    {
        if (!reinterpret_cast<TitleCatkView *>(&g_GameManager.catkData[spellCardNumber])->AttemptedAny(SHOT_ALL) &&
            spellCardNumber >= SPELLCARD_LAST_WORD_START - 1 && spellCardNumber <= SPELLCARD_LW_YUKARI &&
            !g_GameManager.IsLastWordSpellCardAttempted(spellCardNumber))
